...
The default skips jMunch. To include actual pinned jMunch installations and
fixture retrieval, explicitly declare the license basis that applies to your use:

```bash
# Use only if your use qualifies under the upstream noncommercial terms.
uv run --frozen python scripts/live_tools.py --jmunch-use noncommercial
```

Use `commercial-licensed` instead only when you hold the required commercial
licenses. The workflow's manual `jmunch_use` input accepts the same declaration;
push-triggered runs retain `skip`. No license is granted by this installer.

The opt-in checks read all ten adapters' persisted MCP entries, launch the three
servers through both managed Codex executables and uvx, and index only synthetic
Python, Markdown, and CSV fixtures. Source/document retrieval must contain known
fixture content; data filtering must return the exact expected row and exclude a
nonmatching value. AI summaries and embeddings are disabled. The checks run after
both installation and reinstall. Restore must preserve the fixtures and all three
index stores while returning managed installation directories to their baselines.
This remains server-protocol evidence, not actual AI-client tool use.
...
- jMunch is skipped unless the operator explicitly supplies a valid noncommercial
  or paid commercial license basis. Automatic runs do not make that declaration.
- Restore checks compare every owned file against its pre-install SHA-256 or
  ...
  refuses it without changing any client config. Opt-in jMunch runs additionally
  compare managed directory contents and preserved index stores. This does not
  establish client-plugin restore or behavior after an actual client edits config.
...
The first licensed jMunch lifecycle run exposed another real restore conflict:
Python imports generated bytecode-cache files inside the managed installation
after its backup hash was finalized. The saved server environment now disables
those bytecode writes with `PYTHONDONTWRITEBYTECODE=1`. The regression test imports
a real Python fixture and checks that hash-guarded restore still succeeds. Restore's
content-change checks are not relaxed; existing installations already changed by
runtime writes can still require manual inspection when their restore is refused.
